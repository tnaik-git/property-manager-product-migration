...
parser.add_argument('--config-file', required=True, help='Path to the Excel config file (.xls)')
...
df = pd.read_excel(args.config_file)
...
for index, row in df.iterrows():
    ...
    try:
        ...
        # Check if property is already on the target product — skip if so
        property_id = akam_config.propertyId.replace(",", "")
        contract_id = akam_config.contractId.replace(",", "")
        group_id = akam_config.groupId.replace(",", "")
        prop_url = f"{base_url}/papi/v1/properties/{property_id}"
        prop_query = {
            "contractId": contract_id,
            "groupId": group_id,
            "accountSwitchKey": accountSwitchKey
        }
        prop_response = session.get(prop_url, params=prop_query)
        if prop_response.ok:
            current_product_id = prop_response.json().get('properties', {}).get('items', [{}])[0].get('productId', '')
            target_product_code = f"prd_{product_map[target_product]}"
            if current_product_id == target_product_code:
                logger.info(f"⏭️  Skipping {config} — already on {target_product} ({current_product_id})")
                continue
        else:
            logger.warning(f"⚠️  Could not fetch current product for {config}, proceeding with change anyway.")
